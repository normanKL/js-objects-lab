...
const a = pokemon.filter(pokemon => pokemon.starter)
game.party.push(...a)
...
const b = pokemon.filter(pokemon => pokemon.type ==="grass")

const morePokemons = b.slice(0,3)

game.party.push(...morePokemons)
...
const evolveStarters = (party, pokedex) => {
    const evolutions = {
        1: 2,    
        4: 5,    
        7: 8,    
    ...
    party.forEach((pokemon, index) => {
        if (evolutions[pokemon.number]) {
            const evolvedForm = pokedex.find(p => p.number === evolutions[pokemon.number]);
...
const d = pokemon.filter(pokemon => pokemon.starter)
...
// game.catchPokemon(examplePokemon)
